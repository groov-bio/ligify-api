    ...
    if response.ok:
        ...
        for entry in data:
            if entry["organism"]["lineage"][0] == "Bacteria":
                # Get reference DOIs
                try:
                    description = entry["proteinDescription"]["recommendedName"][
                        "fullName"
                    ]["value"]
                    dois = []
                ...
                try:
                    ncbi_id = [
                        e["id"]
                        for e in entry["uniProtKBCrossReferences"]
                        if e["database"] == "RefSeq"
                    ][0]
                except Exception:
                    try:
                        ncbi_id = [
                        ...
                        ncbi_id = [
                            e["value"] for e in ncbi_id if e["key"] == "ProteinId"
                        ][0]
                    except Exception:
                        ncbi_id = None
